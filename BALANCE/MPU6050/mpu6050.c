...
short last_gyro[3], last_accel[3];
float last_Pitch,last_Roll,last_Yaw;
//Pitch: 俯仰角，前倾为正
//Roll:  侧翻角，左侧为正
//Yaw:   旋转角，逆时针为正

float Deviation_Pitch,Pitch_correct;
...
{	
	  unsigned long sensor_timestamp;
		...
		if(Pitch  >20000 || Pitch  <-20000)Pitch=last_Pitch;
	  if(gyro[0]>10000 || gyro[0]<-10000)gyro[0]=last_gyro[0];
		...
		{    
			 ...
			 //Pitch = Mean_Filter(atan2(2 * q2 * q3 + 2 * q0 * q1, -2 * q1 * q1 - 2 * q2* q2 + 1)* 57.3); // roll
...
float Angle_filtered, Gyro_Balance, Gyro_Turn, Acceleration_Z;
void Angle_filter(int Way_Angle)
{
	...
	if(Way_Angle==1)		  	Angle_filtered=Kalman_Filter(Accel_Angle,Gyro_X); //卡尔曼滤波	
	else if(Way_Angle==1)   Angle_filtered=Yijielvbo(Accel_Angle,Gyro_X);     //互补滤波
